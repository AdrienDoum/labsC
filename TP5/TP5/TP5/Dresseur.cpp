...
{
	FoncteurCreaturesDeMemeNom test(nom);
	if (find_if(creatures_.begin(), creatures_.end(), test) == creatures_.end())
		return nullptr;
	return *find_if(creatures_.begin(), creatures_.end(), test);
}
...
bool Dresseur::operator==(const Dresseur& dresseur) const //A compléter
{
	if (creatures_.size() == 0 && dresseur.creatures_.size() == 0)
		...
		for (auto it = creatures_.begin(); it != creatures_.end(); it++) {
			...
			if (find_if(dresseur.creatures_.begin(), dresseur.creatures_.end(), comparaison) == creatures_.end())
				return false;
		}
		return true;
	}
